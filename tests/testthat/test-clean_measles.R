...
test_that("clean_measles works correctly on monthly dataset", {
  result <- clean_measles(cases_month)
...
test_that("clean_measles works correctly on yearly dataset", {
  result <- clean_measles(cases_year)
  expect_true(is.data.frame(result))
